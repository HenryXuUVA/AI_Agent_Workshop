...
class AgentState(TypedDict):
    ...
    - active_model: Which model handled the current input
    - llama_response: Response generated by Llama
    - qwen_response: Response generated by Qwen

    State Flow:
    1. Initial state: all fields empty/default
    2. After get_user_input: user_input and should_exit are populated
    3. After call_llama/call_qwen: selected response field is populated
    4. After print_response: state unchanged (node only reads, doesn't write)

    The graph loops continuously:
        get_user_input -> [conditional] -> call_llama/call_qwen -> print_response -> get_user_input
                              |
...
def create_graph(llama_llm, qwen_llm):
    """
    Create the LangGraph state graph with model-selection routing:
    1. get_user_input: Reads input from stdin
    2. call_llama: Sends input to Llama model
    3. call_qwen: Sends input to Qwen model
    4. print_response: Prints the selected model response

    Graph structure with conditional routing and internal loop:
        START -> get_user_input -> [conditional] -> call_llama ------+
    ...
    #   - model responses: Unchanged (not modified by this node)
    def get_user_input(state: AgentState) -> dict:
        ...
        print("\n" + "=" * 50)
        print(
            "Enter your text ('Hey Qwen ...' to route to Qwen, "
    ...
    def call_llama(state: AgentState) -> dict:
        ...
        # Store which model handled this input and clear stale branch output.
        return {
            "active_model": "llama",
    ...
    # =========================================================================
    # This node reads the selected model response from state and prints it.
    # State changes:
    #   - No changes (this node only reads state, doesn't modify it)
    def print_response(state: AgentState) -> dict:
        """
        Node that prints the selected model response to stdout.

        Reads state:
            - active_model: Which model handled the input
            - llama_response / qwen_response: Model outputs
        Updates state:
        ...
        model = state.get("active_model", "llama")
        if model == "qwen":
            print("\n" + "-" * 50)
            print("Qwen Response:")
            print("-" * 50)
    ...
    #   3. User entered non-empty normal input -> proceed to a selected model
    def route_after_input(state: AgentState) -> str:
        ...
        # Route to Qwen only when input begins with "Hey Qwen" (case-insensitive).
        if state.get("user_input", "").strip().lower().startswith("hey qwen"):
            return "call_qwen"

        # Default route goes to Llama.
        return "call_llama"
    ...
    # 2. get_user_input -> [conditional] -> selected model/get_user_input/END
...
def main():
    ...
    - call_llama/call_qwen: Processes input through one selected model
    ...
    #   - selected model node populates response
